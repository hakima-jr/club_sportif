// src/pages/admin/coachs/ProfilsCoachs.js
...
function CoachCard({ coach, onToggle, onDelete, navigate }) {
  ...
  return (
    <div style={{ background:C.surface, borderRadius:20, border:`1px solid ${C.border}`, overflow:'hidden', display:'flex', flexDirection:'column', transition:'all 0.3s ease', boxShadow:'0 4px 20px rgba(0,0,0,0.3)' }}
      ...
      <div style={{ padding:'22px 22px 16px', display:'flex', alignItems:'center', gap:16 }}>
        <Avatar coach={coach} size={64} />
        <div style={{ flex:1, minWidth:0 }}>
          <p style={{ margin:0, fontWeight:700, fontSize:16, color:C.text, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{coach.prenom} {coach.nom}</p>
          <p style={{ margin:'4px 0 0', fontSize:12, color:C.textMuted }}>{coach.sexe||'—'}</p>
        </div>
        <button onClick={()=>onToggle(coach)} style={{ padding:'5px 14px', borderRadius:20, fontSize:11, fontWeight:700, cursor:'pointer', border:'1px solid', whiteSpace:'nowrap', background:isActif?'rgba(34,197,94,0.15)':'rgba(239,68,68,0.15)', borderColor:isActif?'#22c55e':'#ef4444', color:isActif?'#22c55e':'#ef4444' }}>
      ...
      {spec && (
        <div style={{ padding:'0 22px 14px' }}>
          <span style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'5px 14px', borderRadius:20, fontSize:11, fontWeight:700, background:col.bg, color:col.color, border:`1px solid ${col.border}` }}>
            <Icon d={IC.medal} size={11} color={col.color} /> {spec}
          </span>
        </div>
      )}
      <div style={{ height:1, background:C.border, margin:'0 22px' }} />
      <div style={{ padding:'16px 22px', display:'flex', flexDirection:'column', gap:10, flex:1 }}>
        {[
          ...
          <div key={r.iconKey} style={{ display:'flex', gap:10, alignItems:'flex-start' }}>
            <Icon d={IC[r.iconKey]} size={14} color="#64748b" />
            <span style={{ fontSize:13, color:r.color, wordBreak:'break-all', lineHeight:1.5 }}>{r.val}</span>
          </div>
        ))}
        {coach.salaire && (
          <div style={{ display:'flex', alignItems:'center', gap:10, marginTop:6, padding:'10px 14px', background:'rgba(34,197,94,0.08)', borderRadius:10, border:'1px solid rgba(34,197,94,0.2)' }}>
            <Icon d={IC.money} size={16} color={C.success} />
            <span style={{ color:C.success, fontWeight:700, fontSize:15 }}>{fmt(coach.salaire)} MAD / mois</span>
          </div>
        )}
      </div>
      <div style={{ padding:'14px 22px 20px', display:'flex', gap:10 }}>
        <button onClick={()=>navigate(`/admin/coachs/${coach.id_coach}`)} style={{ flex:2, padding:'10px 0', background:C.surfaceAlt, border:`1px solid ${C.borderLight}`, borderRadius:10, color:C.textLight, fontSize:13, cursor:'pointer', fontWeight:600, transition:'all 0.2s', display:'flex', alignItems:'center', justifyContent:'center', gap:4 }}
        ...
        <button onClick={()=>navigate(`/admin/coachs/modifier/${coach.id_coach}`)} style={{ flex:2, padding:'10px 0', background:'rgba(59,130,246,0.1)', border:'1px solid rgba(59,130,246,0.3)', borderRadius:10, color:C.info, fontSize:13, cursor:'pointer', fontWeight:600, transition:'all 0.2s', display:'flex', alignItems:'center', justifyContent:'center', gap:4 }}
          onMouseEnter={e=>e.currentTarget.style.background='rgba(59,130,246,0.2)'}
          onMouseLeave={e=>e.currentTarget.style.background='rgba(59,130,246,0.1)'}>
          <Icon d={IC.edit} size={12} color={C.info} /> Modifier
        </button>
        <button onClick={()=>onDelete(coach.id_coach)} style={{ flex:1, padding:'10px 0', background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.3)', borderRadius:10, color:C.danger, fontSize:13, cursor:'pointer', fontWeight:600, display:'flex', alignItems:'center', justifyContent:'center' }}
...
export default function ProfilsCoachs() {
  ...
  return (
    <div style={{ padding:'0 0 60px', fontFamily:'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', maxWidth:1400, margin:'0 auto', background:C.bgDark, minHeight:'100vh' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:24 }}>
          <div>
            <h1 style={{ fontSize:28, fontWeight:800, color:'#f1f5f9', margin:0, letterSpacing:'-0.5px', display:'flex', alignItems:'center', gap:12 }}>
              <Icon d={IC.users} size={26} color="#a78bfa" /> Profils des Coachs
            </h1>
            <p style={{ color:'rgba(255,255,255,.5)', marginTop:8, fontSize:14 }}>Gérez l'équipe d'entraîneurs de votre club</p>
          </div>
          <div style={{ display:'flex', gap:16, alignItems:'center', flexWrap:'wrap' }}>
            <div style={{ display:'flex', gap:12 }}>
              {[
                ...
                { num:coachs.length-actifs,label:'Inactifs',     col:'#f87171', bg:'rgba(248,113,113,0.15)' },
                ...
                <div key={s.label} style={{ background:s.bg, borderRadius:14, padding:'14px 20px', textAlign:'center', minWidth:80, border:`1px solid ${s.col}30` }}>
                  <span style={{ display:'block', fontSize:s.label==='Salaires/mois'?13:22, fontWeight:800, color:s.col }}>{s.num}</span>
      ...
      {/* Toast */}
      {message.text && (
        <div style={{ margin:'0 40px 20px', padding:'14px 20px', borderRadius:12, border:'1px solid', fontSize:14, fontWeight:600, display:'flex', alignItems:'center', gap:10,
      ...
      <div style={{ display:'flex', gap:16, margin:'0 40px 24px', flexWrap:'wrap', alignItems:'center' }}>
        ...
        <select value={filterSpec} onChange={e=>setFilterSpec(e.target.value)} style={{ ...inputStyle, minWidth:180, cursor:'pointer' }}>
          <option value="all">Toutes spécialités</option>
          {['Musculation','Cardio','Yoga','Boxe','Natation','Zumba','Fitness','Arts martiaux'].map(s=><option key={s} value={s}>{s}</option>)}
        </select>
      </div>

      <p style={{ color:C.textMuted, fontSize:13, margin:'0 40px 20px', fontWeight:500 }}>{filtered.length} coach(s) trouvé(s)</p>

      {/* Grid */}
      {loading ? (
        <div style={{ display:'flex', justifyContent:'center', padding:'80px 0', color:C.textMuted, fontSize:16 }}>
          <div style={{ textAlign:'center' }}><Icon d={IC.users} size={40} color={C.textMuted} /><p style={{ marginTop:16 }}>Chargement des coachs…</p></div>
        </div>
      ) : filtered.length === 0 ? (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'100px 0', textAlign:'center' }}>
          ...
          <p style={{ color:C.textMuted, margin:'20px 0 0', fontSize:16, fontWeight:500 }}>Aucun coach trouvé</p>
          <p style={{ color:C.textMuted, margin:'8px 0 0', fontSize:13 }}>Essayez de modifier votre recherche</p>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(320px, 1fr))', gap:24, padding:'0 40px' }}>
          {filtered.map(c => <CoachCard key={c.id_coach} coach={c} onToggle={toggleStatut} onDelete={handleDelete} navigate={navigate} />)}
